...
The output has exactly these eight columns:

```csv
request_id,amount_safe_to_pay,affordability_status,recommended_payment_method,payment_plan,earliest_date_for_full_payment,spending_changes_needed,decision_explanation
```

Allowed statuses are `affordable_now`, `affordable_with_plan`, `affordable_later`, and `not_affordable`; methods are `full_payment`, `partial_payment`, `installments`, `wait`, and `not_recommended`. Use chronological `YYYY-MM-DD:amount` payments joined by `|`, or `none`; use up to three `stop:event_id` or `reduce_to:event_id:new_amount` actions joined by `|`, or `none`. The earliest-full date is a date or empty, not `none`. [^L2]

The output combines a number, four possible affordability statuses, five possible methods, dated payments, a capacity date, spending actions, and an explanation. A binary “buy/wait” model discards most of the evaluated contract. Even separate regressors/classifiers would still need a common financial model to keep their outputs consistent. (inference) [^L2]
...
That is a useful introduction, but the picture assumes that the prediction target, training examples, and meaning of success are already defined. It omits time, financial constraints, label availability, and structured schedules. “We need a yes/no answer” is not sufficient evidence that logistic regression should control this agent. (interpretation) [^I1][^S1]
...
If a pending debit is reserved immediately for capacity, do not deduct it a second time when its settlement is displayed. Use an explicit reserved-versus-settled representation. An explicit confirmed occurrence must replace its matching inferred recurrence occurrence, not be added to it as a second obligation. For same-day flows without a supported order, use and document a conservative ordering rather than spending an income credit early. (design inference) [^L2][^L4]

### D. Calculate capacity, then simulate plans

Let `C(t)` be the projected no-purchase, no-optional-change available balance after the normalized baseline cash events; let `M` be the protected minimum, `A` the requested amount, `r` the request date, and `H` the end of the request-anchored 90-day forecast.
...
- **Full payment:** require acceptance of `full_payment` and simulate the full amount on candidate dates, both without changes and with permitted changes where needed. Full payment today enabled only by spending changes is `affordable_with_plan`, not `affordable_now`; public samples 06, 11 and 21 demonstrate this distinction.
- **Partial payment:** both request and user must permit it; require `0 < safe_today < A`; pay exactly `safe_today` on the request date and the remainder on `earliest_date_for_full_payment`, no later than the desired date. No seller partial-payment offer is required. Re-simulate both payments; a safe full-payment date alone does not prove the preceding partial payment harmless.
- **Installments:** construct the supplied offer unchanged from `first_payment_date`, `payment_frequency_days`, `number_of_payments`, and `payment_amount`. Include financing costs; respect user preference and installment limit; reject late or unsafe offers. A 30-day interval is not necessarily a calendar month. Preserve supplied per-payment amounts; compare their Decimal sum with the declared total and document any rounding discrepancy rather than silently changing the final installment or weakening balance safety.
- **Spending changes:** consider at most three permitted, non-protected recurring changes. `reduce_to` requires `reducible` or `reducible_or_stoppable` flexibility, the user's permitted reduction category, and the supplied `minimum_allowed_amount` floor. `stop` requires `stoppable` or `reducible_or_stoppable` flexibility and the user's permitted stopping category. Every reducible event in this dataset has a floor; the blank floors belong to non-reducible types, not permission to reduce to zero. Stopping and reducing the same event are mutually exclusive. Re-simulate affected future occurrences, not a refund of historical spending.
- **Wait:** require a later safe full payment, acceptance of `full_payment`, and completion on or before `desired_completion_date`. Every recommended plan, including wait, must satisfy the deadline. A later no-change capacity date may still be reported independently, but must not be recommended as an on-time payment when it misses the deadline.
- **Fallback:** use `not_recommended` when no safe eligible payment exists; keep financial-capacity fields distinct from preference eligibility. Never empty an otherwise valid capacity date merely to make it agree with the selected method. [^L2][^L3][^L4]
...
Public samples 06, 11 and 21 show why these fields must stay separate: permitted spending changes enable full payment today even though no-change capacity is lower and the no-change earliest-full date is later. This is public format evidence, not a proposed answer for an evaluation request. [^L4]

Render explanations from the validated ledger: what is reserved, which confirmed income matters, which option is selected, the lowest projected balance, and any allowed change. The explanation should be a readable summary of actual calculations, not a second AI decision that can contradict them. (design inference) [^L2]
...
3. **Supervised adaptation later:** learn a narrowly defined target from permissioned, quality-controlled labels. (design distinction) [^L2][^S1][^S8]

If the future target is plan preference, collect request-time features, the feasible alternatives shown, the selected action, and separately defined later feedback. A click or purchase is not automatically a “good financial decision” label. Use only features available when the decision was made; an already-known future salary schedule is legitimate context, but a later-realized refund outcome is not a request-time feature. (proposed label policy) [^S4]

Candidate features might include requested-amount-to-surplus ratio, minimum projected headroom, days until confirmed income, deadline slack, fee-inclusive cost, number of payments, and explicit method/priority flags. Define ratios safely when surplus is zero, normalize currency-sensitive magnitudes, and fit preprocessing only on training partitions. These are proposed features, not an experimentally selected set. (proposal) [^S1][^S4]
...
- explanations refer only to validated facts. (proposed tests) [^L2][^L3][^L4]
...
The required usage report must correspond to the final run that produced the submitted `output.csv`: exact provider/model names, calls, input/output tokens, totals and per-request averages, estimated total/per-request costs, and per-model plus overall totals if multiple models are used. Include retries and shared extraction calls in full-run totals; allocate shared cost transparently. Keep research/coding-assistant usage separate from application inference. Prefer one clearly accounted cold-cache final run. If a run uses previously model-extracted cached facts, disclose that provenance and distinguish final-run incurred calls from the cost of creating the cache; zero new calls does not mean the solution never used a model. If the application truly uses no token-billed models, report that honestly rather than inventing token usage. (accounting proposal) [^L2]
...
Exclude credentials, `.env`, `.git`, organizer-only files, hidden labels, unnecessary financial-data copies, and private research/operational artifacts. If dataset assets are required inside the ZIP, include only permitted participant assets and document their placement; portal-specific inclusion and size requirements remain unverified. (packaging recommendation) [^L2]

**Deadline stated by the project:** 13 September 2026, 18:00 IST. This is the documented deadline, not a verified portal acceptance window. [^L3]
...
**High confidence:** a dated, constraint-checking financial core is necessary; the output is not binary; public examples are sparse; blank amounts require evidence extraction; model outputs cannot override the contract. [^L1][^L2][^L3][^L4]

**Verification: BLOCKED** for complete access checks. The portal returned Access Denied; reopening the supplied JPEG during the final check returned `EPERM`. Earlier image inspection and its recorded description remain the basis for section 2. No access-control workaround was attempted. [^I1][^V1]
...
- currency precision/rounding and same-day debit/credit ordering; the local option-total check used a one-cent comparison and did not prove exact Decimal equality for all declared totals, so preserve schedules and explicitly resolve discrepancies without relaxing the minimum-balance rule;
...
Local paths are relative to the repository root. Local evidence is pinned to commit `963ad7eb3d058ace2bf2e8bf4324472c981f0840`; dataset aggregates are reported by the local investigation, not produced by a trained model. External sources were retrieved on 12 September 2026. Paper use is limited to metadata and HTML abstracts, not full-paper or code audits. No PDF was fetched or parsed.

[^L1]: Local `README.md`: lines 29–55 (terminal command and root output), 92–129 (fields/workflow), 138–193 (evaluation, usage accounting, transcript and submission). Read in full.
[^L2]: Local `problem_statement.md`: lines 15–49 (scope/inputs), 82–163 (output semantics), 165–213 (evidence, 90-day safety, ranking, conflicts and investment scope), 215–251 (evaluation/submission/usage). Read in full. It is the primary financial/output contract; its template-location wording conflicts with the more explicit README, as noted above.
[^L3]: Local `AGENTS.md`: sections 2–5 (logging, deadline and mandatory submission URL), 6.1–6.5 (permitted data, financial/output rules and artifacts). Read in full.
[^L4]: `outputs/.drafts/buy-wait-agent-research-local.md`, complete local inventory, exact schemas, aggregate commands/results, bounded image findings, empty-entry-point evidence and limitations. Source artifacts are the named participant files under `dataset/` and `code/main.py`. Only one dataset image's content was inspected; all 16 image paths and event links were checked.
[^I1]: Supplied file `/Users/and3/Downloads/WhatsApp Image 2026-09-12 at 07.59.23.jpeg`, a 1132×1552 graphic titled “ALL FIVE, ON ONE PLANE.” Earlier successful inspection is recorded for the local investigator and lead; the investigator transcribed the five captions. A later lead attempt to reopen the same file returned `EPERM`, so repeat visual verification is blocked. The earlier evidence reports legible captions and a cropped lower social-media interface; no claim is made about content outside the visible image.
[^S1]: scikit-learn, [Linear Models: ordinary least squares and logistic regression](https://scikit-learn.org/stable/modules/linear_model.html#logistic-regression). Official documentation excerpts and relevant HTML sections read. Supports targets, objectives, probabilities and regularization—not a challenge-specific result.
[^S2]: scikit-learn, [Probability calibration](https://scikit-learn.org/stable/modules/calibration.html). Official HTML read. Supports reliability evaluation, fitting/calibration separation and the limits of treating Brier/log loss as calibration-only measures.
[^S3]: scikit-learn, [Cross-validation: evaluating estimator performance](https://scikit-learn.org/stable/modules/cross_validation.html). Official HTML read; group/temporal separation and the pitfalls of same-data evaluation were checked.
[^S4]: scikit-learn, [Common pitfalls and recommended practices](https://scikit-learn.org/stable/common_pitfalls.html). Official HTML read; supports request-time information availability, leakage prevention and train-only preprocessing.
[^S5]: Adam N. Elmachtoub and Paul Grigas, [Smart “Predict, then Optimize”](https://arxiv.org/abs/1710.08005v5), arXiv:1710.08005v5 (2017; version updated 2020). Official metadata and HTML abstract read. Cited for the distinction between prediction and downstream decision objectives, not as empirical household-budget evidence.
[^S6]: Léo Grinsztajn, Edouard Oyallon and Gaël Varoquaux, [Why do tree-based models still outperform deep learning on typical tabular data?](https://arxiv.org/abs/2207.08815v1), arXiv:2207.08815v1 (2022). Official metadata and HTML abstract read. Its benchmark scope limits the conclusion.
[^S7]: Noah Hollmann et al., [Accurate predictions on small data with a tabular foundation model](https://doi.org/10.1038/s41586-024-08328-6), *Nature* 637:319–326 (2025). DOI resolved to the publisher HTML and its abstract was read. Verified metadata: PMID **39780007**, PMCID **PMC11711098**, OpenAlex **W4406170795**, venue **S137773608**. The PubMed HTML page was blocked (403), but the exact PubMed API lookup succeeded. No benchmark numbers are transferred to this challenge.
[^S8]: Noah Hollmann et al., [TabPFN: A Transformer That Solves Small Tabular Classification Problems in a Second](https://arxiv.org/abs/2207.01848v6), arXiv:2207.01848v6 (initial 2022). Official metadata and HTML abstract read. Cited for supervised prediction conditioned on labeled examples, not for a guaranteed speed on this project.
[^V1]: `outputs/.drafts/buy-wait-agent-research-direct.md`, exact-query ledger, accepted/rejected source records, endpoint provenance and direct HTML-verification table. The mandated submission URL returned an Access Denied page. No portal workflow, upload limits, authentication, account access or submission was verified.
